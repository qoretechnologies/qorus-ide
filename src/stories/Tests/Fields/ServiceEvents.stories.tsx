...
import { _testsSelectItemFromCollection, sleep } from '../utils';
...
export const CreateNew: StoryObj<typeof meta> = {
  play: async ({ canvasElement, ...rest }) => {
    ...
    await expect(
      document.querySelectorAll('.service-event-handler').length
    ).toBe(2);
...
export const ModifyExisting: StoryObj<typeof meta> = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await fireEvent.click(
      document.querySelectorAll('.service-event-remove')[2]
    );
    ...
    await fireEvent.click(
      document.querySelectorAll('.service-event-handler-remove')[0]
    );
