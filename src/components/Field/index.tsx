...
const Field = ({ type, interfaceId, interfaceKind, ...rest }: IFieldProps) => {
  ...
  useMount(() => {
    if (rest.value && rest.on_change) {
      ...
      const onChange: string[] = isArray(rest.on_change)
        ? rest.on_change
        : [rest.on_change];
    ...
    <>
      ...
      {type === 'auto' && (
        <AutoField
          {...rest}
          type={type}
          isConfigItem={interfaceKind === 'config-item'}
        />
      )}
      {type === 'array-auto' && <ArrayAutoField {...rest} type={type} />}
      {['number', 'float'].includes(type) && (
        <NumberField {...rest} type={type} />
      )}
      ...
      {type === 'service-events' && (
        <ServiceEventListField {...rest} type={type} />
      )}
      {type === 'options' && (
        <Options
          {...rest}
          onChange={(name, value) => rest.onChange(name, value)}
        />
      )}
      {type === 'url' && <URLField {...rest} type={type} />}
      {type === 'api-manager' && <ApiManager {...rest} />}
      {rest.markdown && rest.value ? (
        <MarkdownPreview value={rest.value} />
      ) : null}
