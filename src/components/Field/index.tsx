...
import { ServiceWebhooksField } from './serviceWebhooks';
...
const Field = ({ type, interfaceId, interfaceKind, ...rest }: IFieldProps) => {
  ...
  return (
    <>
      ...
      {type === 'boolean' || type === 'bool' ? <BooleanField {...rest} type={type} /> : null}
      ...
      {type === 'service-webhooks' && <ServiceWebhooksField {...rest} />}
      {type === 'options' && (
        <Options {...rest} onChange={(name, value) => rest.onChange(name, value)} />
      )}
